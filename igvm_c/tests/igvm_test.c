...
void test_valid_igvm(void) {
	IgvmFile *igvm = NULL;
	CU_ASSERT_EQUAL(igvm_new_from_binary(igvm_buf, igvm_buf_length, &igvm), IGVMAPI_OK);
	CU_ASSERT_PTR_NOT_NULL(igvm);
	if (igvm) {
		igvm_free(igvm);
	}
}
...
void test_invalid_fixed_header(void) {
	IgvmFile *igvm = NULL;
	CU_ASSERT_EQUAL(igvm_new_from_binary(invalid_igvm_buf, sizeof(invalid_igvm_buf), &igvm), IGVMAPI_INVALID_FIXED_HEADER);
	CU_ASSERT_PTR_NULL(igvm);
}
...
void test_platform_header(void) {
	...
	CU_ASSERT_EQUAL(igvm_new_from_binary(igvm_buf, igvm_buf_length, &igvm), IGVMAPI_OK);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_PLATFORM, 0, NULL, &header_length), IGVMAPI_BUFFER_TOO_SMALL);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_PLATFORM, 0, test_buf, &header_length), IGVMAPI_OK);

	header = (IGVM_VHS_VARIABLE_HEADER *)test_buf;
	CU_ASSERT_EQUAL(header->typ, IGVM_VHT_SUPPORTED_PLATFORM);
	CU_ASSERT_EQUAL(header->length, sizeof(IGVM_VHS_SUPPORTED_PLATFORM));

	platform = (IGVM_VHS_SUPPORTED_PLATFORM *)(test_buf + sizeof(IGVM_VHS_VARIABLE_HEADER));
...
void test_initialization_header(void) {
	...
	CU_ASSERT_EQUAL(igvm_new_from_binary(igvm_buf, igvm_buf_length, &igvm), IGVMAPI_OK);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_INITIALIZATION, 0, NULL, &header_length), IGVMAPI_BUFFER_TOO_SMALL);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_INITIALIZATION, 0, test_buf, &header_length), IGVMAPI_OK);
	...
	policy = (IGVM_VHS_GUEST_POLICY *)(test_buf + sizeof(IGVM_VHS_VARIABLE_HEADER));
	...
	/* Second entry */
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_INITIALIZATION, 1, test_buf, &header_length), IGVMAPI_OK);
...
void test_directive_header(void) {
	...
	CU_ASSERT_EQUAL(igvm_new_from_binary(igvm_buf, igvm_buf_length, &igvm), IGVMAPI_OK);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 1, NULL, &header_length), IGVMAPI_BUFFER_TOO_SMALL);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 1, test_buf, &header_length), IGVMAPI_OK);

	header = (IGVM_VHS_VARIABLE_HEADER *)test_buf;
	CU_ASSERT_EQUAL(header->typ, IGVM_VHT_PAGE_DATA);
	CU_ASSERT_EQUAL(header->length, sizeof(IGVM_VHS_PAGE_DATA));
	page = (IGVM_VHS_PAGE_DATA *)(test_buf + sizeof(IGVM_VHS_VARIABLE_HEADER));
	...
	header_length = 0;
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 8, NULL, &header_length), IGVMAPI_BUFFER_TOO_SMALL);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 8, test_buf, &header_length), IGVMAPI_OK);
	header = (IGVM_VHS_VARIABLE_HEADER *)test_buf;
	CU_ASSERT_EQUAL(header->typ, IGVM_VHT_PARAMETER_AREA);
	CU_ASSERT_EQUAL(header->length, sizeof(IGVM_VHS_PARAMETER_AREA));
	param_area = (IGVM_VHS_PARAMETER_AREA *)(test_buf + sizeof(IGVM_VHS_VARIABLE_HEADER));
	...
	header_length = 0;
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 9, NULL, &header_length), IGVMAPI_BUFFER_TOO_SMALL);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 9, test_buf, &header_length), IGVMAPI_OK);
	header = (IGVM_VHS_VARIABLE_HEADER *)test_buf;
	CU_ASSERT_EQUAL(header->typ, IGVM_VHT_VP_COUNT_PARAMETER);
	CU_ASSERT_EQUAL(header->length, sizeof(IGVM_VHS_PARAMETER));
	param = (IGVM_VHS_PARAMETER *)(test_buf + sizeof(IGVM_VHS_VARIABLE_HEADER));
	CU_ASSERT_EQUAL(param->parameter_area_index, 0);
	CU_ASSERT_EQUAL(param->byte_offset, 0);

	header_length = 0;
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 10, NULL, &header_length), IGVMAPI_BUFFER_TOO_SMALL);
	CU_ASSERT_EQUAL(igvm_get_header(igvm, HEADER_SECTION_DIRECTIVE, 10, test_buf, &header_length), IGVMAPI_OK);
	header = (IGVM_VHS_VARIABLE_HEADER *)test_buf;
	CU_ASSERT_EQUAL(header->typ, IGVM_VHT_PARAMETER_INSERT);
	CU_ASSERT_EQUAL(header->length, sizeof(IGVM_VHS_PARAMETER_INSERT));
	ins = (IGVM_VHS_PARAMETER_INSERT *)(test_buf + sizeof(IGVM_VHS_VARIABLE_HEADER));
...
void test_associated_data(void) {
	...
	CU_ASSERT_EQUAL(igvm_new_from_binary(igvm_buf, igvm_buf_length, &igvm), IGVMAPI_OK);
	CU_ASSERT_EQUAL(igvm_get_header_data(igvm, HEADER_SECTION_DIRECTIVE, 3, NULL, &data_length), IGVMAPI_BUFFER_TOO_SMALL);
	CU_ASSERT_EQUAL(igvm_get_header_data(igvm, HEADER_SECTION_DIRECTIVE, 3, test_buf, &data_length), IGVMAPI_OK);
...
void test_no_associated_data(void) {
	...
	CU_ASSERT_EQUAL(igvm_new_from_binary(igvm_buf, igvm_buf_length, &igvm), IGVMAPI_OK);
	CU_ASSERT_EQUAL(igvm_get_header_data(igvm, HEADER_SECTION_DIRECTIVE, 9, NULL, &data_length), IGVMAPI_NO_DATA);
